...
object ptransformGroupOps {
  ...
  def main(cmdlineArgs: Array[String]): Unit = {
    ...
    val min_joined_date_grouped_by_dept = flat_join
      .groupByKey
      .map(x=> {
    ...
    val min_joined_date_grouped_by_dept_2 = flat_join
      .reduceByKey( (x,y) => {
    ...
    val min_joined_date_grouped_by_dept_3 = flat_join
      .aggregateByKey("9999-99-99")(CombFunc,CombFunc)

    val min_joined_date_grouped_by_dept_4 = flat_join
      .mapValues(_.asInstanceOf[String])
      .minByKey(Ordering[String])

    min_joined_date_grouped_by_dept_4
      .map(x => println(x))
